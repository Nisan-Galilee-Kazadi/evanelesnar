...
const Home = () => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            try {
                const response = await fetch('http://localhost:5000/api/events?upcoming=true');
    ...
    const videos = [
        { id: 1, title: 'Best Of 2024', views: '2.5M vues', youtubeId: 'dQw4w9WgXcQ' },
        { id: 2, title: 'Sketch Viral', views: '1.8M vues', youtubeId: 'dQw4w9WgXcQ' }
    ];
        ...
        <div className="min-h-screen">
                ...
                <div className="relative z-10 container-custom text-center px-4 pt-32">
                    <div className="animate-slide-up">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                            <Link to="/events" className="btn btn-primary text-lg px-8 py-4 w-full sm:w-auto flex items-center justify-center gap-2">
                                <span>Réserver un billet</span> <FaTicketAlt className="text-3xl" />
                            </Link>
                            <Link to="/about" className="btn btn-outline text-lg px-8 py-4 w-full sm:w-auto">
                                En savoir plus
                            </Link>
                        </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mt-20 max-w-4xl mx-auto">
                            <div className="card-glass text-center">
                                <div className="text-4xl font-bold text-gradient mb-2">{artistInfo.stats.shows}</div>
                                <div className="text-slate-400 text-sm">Spectacles</div>
                            </div>
                            <div className="card-glass text-center">
                                <div className="text-4xl font-bold text-gradient mb-2">{artistInfo.stats.fans}</div>
                                <div className="text-slate-400 text-sm">Fans</div>
                            </div>
                            <div className="card-glass text-center">
                                <div className="text-4xl font-bold text-gradient mb-2">{artistInfo.stats.years}</div>
                                <div className="text-slate-400 text-sm">Années d'expérience</div>
                            </div>
                            <div className="card-glass text-center">
                                <div className="text-4xl font-bold text-gradient mb-2">{artistInfo.stats.awards}</div>
            ...
            <section className="section bg-gradient-to-br from-slate-900 to-slate-950">
                <div className="container-custom">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
                        {videos.map((video) => (
                            <div key={video.id} className="card group cursor-pointer" onClick={() => openVideo(video.youtubeId)}>
                                <div className="relative aspect-video rounded-lg overflow-hidden mb-4">
                                    <img
                                        src={`https://images.unsplash.com/photo-${video.id === 1 ? '1598387181032' : '1516450360452'}-a3103a2db5b3?w=800&h=450&fit=crop`}
                                        alt={`Vidéo ${video.id}`}
                                        className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                                    />
            ...
            <section className="section bg-gradient-to-r from-orange-600 to-red-600">
                <div className="container-custom text-center">
                    ...
                    <Link to="/events" className="btn bg-white text-orange-600 hover:bg-slate-100 text-lg px-10 py-4 inline-flex items-center gap-2">
                        <span>Réserver mes billets</span> <FaTicketAlt className="text-3xl" />
                    </Link>
                </div>
            </section>
