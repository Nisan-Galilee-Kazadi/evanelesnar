...
import { FaSearch, FaChevronDown, FaChevronUp, FaPhone, FaEnvelope, FaCalendar, FaCreditCard } from 'react-icons/fa';
...
const Orders = () => {
    ...
    const handleValidate = async (orderId) => {
        if (!window.confirm('Confirmer la réception du paiement pour cette commande ?')) return;

        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`http://localhost:5000/api/orders/${orderId}/validate`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
    ...
    const filteredOrders = orders.filter(order => {
        ...
        const matchesEvent = eventFilter === 'all' || order.event?._id === eventFilter;
    ...
    const validatedCount = orders.filter(o => o.paymentStatus === 'validated').length;
            ...
            <div className="p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                    ...
                    <div className="bg-slate-900 border border-green-500/20 rounded-xl p-6">
                        ...
                        <div className="text-3xl font-bold text-green-500">{validatedCount}</div>
                    </div>
                </div>
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4">
                                            ...
                                            <div>
                                                <div className="text-slate-400 text-xs mb-1">Événement</div>
                                                <div className="text-white">{order.event?.title || 'Supprimé'}</div>
                                            </div>
                                            <div>
                                                <div className="text-slate-400 text-xs mb-1">Total</div>
                                                <div className="text-purple-400 font-bold text-lg">{order.totalAmount.toLocaleString()} CDF</div>
                                            </div>
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    <div className="text-slate-400 text-xs mb-1">Statut</div>
                                                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${order.paymentStatus === 'validated'
                                                        ? 'bg-green-500/10 text-green-500'
                                                        : 'bg-yellow-500/10 text-yellow-500'
                                                        }`}>
                                                        {order.paymentStatus === 'validated' ? 'VALIDÉ' : 'EN ATTENTE'}
                                                    </span>
                                                </div>
                                    ...
                                    <div className="border-t border-slate-800 p-6 bg-slate-950">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <div>
                                                ...
                                                <div className="space-y-2 text-sm">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span className="text-white font-mono">{order._id.slice(-8)}</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                        <FaCreditCard className="text-slate-400" />
                                                        <span className="text-white">{getPaymentMethodName(order.paymentMethod)}</span>
                                                    </div>
                                                    <div className="flex items-center gap-2">
                                                    ...
                                                    {order.token && (
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-green-500 font-mono font-bold">{order.token}</span>
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="md:col-span-2">
                                                <h3 className="text-white font-semibold mb-4">Billets</h3>
                                                <div className="bg-slate-900 rounded-lg p-4">
                                                    {order.tickets.map((ticket, idx) => (
                                                        <div key={idx} className="flex justify-between items-center py-2 border-b border-slate-800 last:border-0">
                                                            <span className="text-white">{ticket.quantity}x {ticket.type}</span>
                                                            <span className="text-purple-400 font-semibold">
                                                                {(ticket.price * ticket.quantity).toLocaleString()} CDF
                                                            </span>
                                                        </div>
                                                    ))}
