...
const EventsManager = () => {
    ...
    const addTicketType = () => {
        setFormData({
            ...formData,
            tickets: [...formData.tickets, { type: 'VIP', price: 0, available: 50, total: 50 }]
        });
    };
        ...
        <AdminLayout>
            <div className="p-8">
                <div className="mb-8">
                    ...
                    <p className="text-slate-400">Créer, modifier ou supprimer des événements</p>
                </div>
                ...
                {!showForm ? (
                    <>
                        ...
                        <div className="grid gap-4">
                            {events.map((event) => (
                                <div key={event._id} className="bg-slate-900 p-6 rounded-xl border border-slate-800 flex justify-between items-center">
                                    <div className="flex gap-4 items-center">
                                        <img src={event.image} alt={event.title} className="w-16 h-16 object-cover rounded-lg" />
                                        <div>
                                            <h3 className="text-xl font-bold text-white mb-1">{event.title}</h3>
                                            <p className="text-slate-400 text-sm">
                                                {new Date(event.date).toLocaleDateString('fr-FR')} à {event.time} • {event.venue}, {event.city}
                                            </p>
                                            <span className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-semibold ${event.status === 'upcoming' ? 'bg-green-500/10 text-green-500' :
                                                event.status === 'soldout' ? 'bg-red-500/10 text-red-500' :
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label className="block text-slate-400 mb-2">Lieu (Salle)</label>
                                    <input
                                        type="text"
                            ...
                            <div>
                                <div className="flex justify-between items-center mb-4">
                                    <label className="block text-slate-400">Types de Billets</label>
                                    <button
                                        type="button"
                                ...
                                <div className="space-y-4">
                                    {formData.tickets.map((ticket, index) => (
                                        <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-slate-950 p-4 rounded border border-slate-800 relative">
                                            <input
                                                type="text"
