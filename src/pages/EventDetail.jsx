...
const EventDetail = () => {
        ...
        return (
            <div className="min-h-screen pt-32 flex items-center justify-center">
                <div className="text-center">
                    <div className="text-6xl mb-4 text-slate-600 flex justify-center"><FaRegSadTear /></div>
                    <h2 className="text-3xl font-bold text-white mb-4">Spectacle introuvable</h2>
    ...
    const handleContinue = async () => {
        if (step === 1 && getTotalTickets() > 0) {
            setStep(2);
        } else if (step === 2 && selectedPayment && customerInfo.name && customerInfo.phone) {
            try {
                // Prepare order data
    ...
    const handleValidateToken = async () => {
        ...
        try {
            const response = await fetch('http://localhost:5000/api/orders/verify-token', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                    ...
                    <div>
                        ...
                        <div className="space-y-4 mb-8">
                            <div className="flex items-center space-x-3 text-lg">
                                <span className="text-2xl text-orange-500"><FaCalendarAlt /></span>
                                <div>
                                    ...
                                    <div className="text-slate-400 text-sm">Date du spectacle</div>
                                </div>
                            </div>

                            <div className="flex items-center space-x-3 text-lg">
                                <span className="text-2xl text-orange-500"><FaClock /></span>
                                <div>
                            ...
                            <div className="flex items-center space-x-3 text-lg">
                                <span className="text-2xl text-orange-500"><FaMapMarkerAlt /></span>
                                <div>
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    {step !== 4 && (
                        <div className="flex items-center justify-center mb-12">
                            <div className="flex items-center space-x-4">
                                <div className={`flex items-center space-x-2 ${step >= 1 ? 'text-orange-500' : 'text-slate-600'}`}>
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${step >= 1 ? 'bg-orange-500 text-white' : 'bg-slate-800 text-slate-600'}`}>
                                        1
                                    </div>
                                    <span className="hidden sm:inline font-medium">Billets</span>
                                </div>
                                <div className={`w-16 h-0.5 ${step >= 2 ? 'bg-orange-500' : 'bg-slate-800'}`}></div>
                                <div className={`flex items-center space-x-2 ${step >= 2 ? 'text-orange-500' : 'text-slate-600'}`}>
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${step >= 2 ? 'bg-orange-500 text-white' : 'bg-slate-800 text-slate-600'}`}>
                                        2
                                    </div>
                                    <span className="hidden sm:inline font-medium">Paiement</span>
                                </div>
                                <div className={`w-16 h-0.5 ${step >= 3 ? 'bg-orange-500' : 'bg-slate-800'}`}></div>
                                <div className={`flex items-center space-x-2 ${step >= 3 ? 'text-orange-500' : 'text-slate-600'}`}>
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${step >= 3 ? 'bg-orange-500 text-white' : 'bg-slate-800 text-slate-600'}`}>
                                        3
                                    </div>
                    ...
                    {step === 4 && pendingOrder && (
                        <div className="card-glass animate-slide-up text-center">
                            <div className="text-6xl mb-6 text-orange-500 flex justify-center"><FaTicketAlt /></div>
                            <h2 className="text-4xl font-bold text-white mb-4">Validez votre Token</h2>
                            ...
                            <div className="bg-slate-800/50 rounded-xl p-6 mb-8 max-w-2xl mx-auto border border-slate-700">
                                ...
                                <p className="text-slate-300 mb-6">
                                    Entrez le token que vous avez reçu après validation du paiement pour télécharger votre billet.
                                </p>
                    ...
                    {step === 1 && (
                        <div className="card-glass animate-slide-up">
                            <h2 className="text-3xl font-bold text-white mb-8">Sélectionnez vos billets</h2>

                            <div className="space-y-6 mb-8">
                                {event.tickets.map((ticket, idx) => (
                                    <div key={idx} className="card flex flex-col sm:flex-row items-center justify-between gap-4">
                                        <div className="flex-1">
                                            <h3 className={`text-xl font-bold mb-2 flex items-center gap-2 ${ticket.type === 'VIP' ? 'text-gradient-gold' : 'text-white'}`}>
                                                {ticket.type === 'VIP' && <FaStar className="text-yellow-500" />}{ticket.type}
                                            </h3>
                                        ...
                                        <div className="flex items-center space-x-4">
                                            <button
                                                onClick={() => handleTicketChange(ticket.type, (selectedTickets[ticket.type] || 0) - 1)}
                            ...
                            <div className="border-t border-slate-700 pt-6 mb-6">
                                <div className="flex items-center justify-between text-2xl font-bold mb-2">
                                    ...
                                    <span className="text-gradient">{getTotalAmount().toLocaleString()} CDF</span>
                                </div>
                    ...
                    {step === 2 && (
                        <div className="card-glass animate-slide-up">
                            <h2 className="text-3xl font-bold text-white mb-8">Informations et Paiement</h2>
                            ...
                            {/* Payment Methods */}
                            <div className="mb-8">
                                <h3 className="text-xl font-bold text-white mb-4">Choisissez votre méthode de paiement</h3>
                                <div className="grid grid-cols-4 gap-2 md:gap-4">
                                    {paymentMethods.map((method) => {
                                        const isRedBg = method.id === 'mpesa' || method.id === 'airtel';
                                        return (
                                            <div
                                                key={method.id}
                                                onClick={() => setSelectedPayment(method.id)}
                                                className={`payment-icon ${selectedPayment === method.id ? 'active' : ''} ${isRedBg ? '!bg-red-600' : ''}`}
                                            >
                                                <img
                                                    src={method.logo}
                                                    alt={method.name}
                                                    className={`w-full h-full object-contain ${isRedBg ? 'brightness-0 invert' : ''}`}
                                                />
                                            </div>
                                        );
                                    })}
                            ...
                            <div className="card mb-6">
                                ...
                                {Object.entries(selectedTickets).map(([type, qty]) => {
                                    ...
                                    const ticket = event.tickets.find(t => t.type === type);
                                    return (
                                        <div key={type} className="flex justify-between text-slate-400 mb-2">
                    ...
                    {step === 3 && (
                        <div className="card-glass text-center animate-slide-up">
                            <div className="text-6xl mb-6 text-orange-500 flex justify-center"><FaMobileAlt /></div>
                            <h2 className="text-4xl font-bold text-white mb-4">Demande enregistrée !</h2>
                            ...
                            <div className="bg-slate-800/50 rounded-xl p-6 mb-8 max-w-2xl mx-auto border border-slate-700">
                                ...
                                <p className="text-slate-300 leading-relaxed">
                                    Une fois le paiement effectué, vous recevrez votre <span className="text-white font-bold">token unique</span> dans les 12h qui suivent pour télécharger votre billet.
                                </p>
                                <p className="text-slate-400 text-sm mt-4">
                                    💡 Revenez sur cette page avec votre token pour télécharger votre billet !
                                </p>
                            </div>
