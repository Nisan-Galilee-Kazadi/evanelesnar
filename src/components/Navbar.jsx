...
const Navbar = () => {
    ...
    return (
        <nav className={`navbar fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-slate-950/90 backdrop-blur-md py-4 shadow-2xl' : 'bg-transparent py-6'}`}>
                ...
                <div className="flex items-center justify-between">
                    ...
                    <Link to="/" className="flex items-center space-x-3 group">
                        <div className="w-12 h-12 bg-gradient-to-br from-orange-500 to-red-600 rounded-lg flex items-center justify-center transform group-hover:rotate-12 transition-transform duration-300 shadow-lg shadow-orange-500/20">
                            <span className="text-2xl text-white"><FaMask /></span>
                        </div>
                        <div>
                            <h1 className="text-2xl font-bold text-white transition-all duration-300 group-hover:scale-110 group-hover:tracking-wider">Evan Lesnar</h1>
                            <p className="text-xs text-orange-500 font-medium tracking-wider">HUMORISTE</p>
                        </div>
                    </Link>
                    ...
                    <div className="hidden md:flex items-center space-x-8">
                        {navLinks.map((link) => (
                            ...
                            >
                                {link.name}
                                <span className={`absolute -bottom-1 left-0 w-0 h-0.5 bg-gradient-to-r from-orange-500 to-red-600 transition-all duration-300 group-hover:w-full ${location.pathname === link.path ? 'w-full' : ''
                                    }`}></span>
                            </Link>
                        ))}
                        <Link to="/events" className="btn btn-primary text-sm flex items-center space-x-2">
                            <span>Réserver</span>
                            <FaTicketAlt />
                        </Link>
                    </div>
                    ...
                    >
                        <span className={`w-6 h-0.5 bg-orange-500 transition-all duration-300 ${isMobileMenuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
                        <span className={`w-6 h-0.5 bg-orange-500 transition-all duration-300 ${isMobileMenuOpen ? 'opacity-0' : ''}`}></span>
                        <span className={`w-6 h-0.5 bg-orange-500 transition-all duration-300 ${isMobileMenuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
                    </button>
                </div>
                ...
                {isMobileMenuOpen && (
                    <div className="md:hidden absolute top-0 left-0 w-full bg-slate-950 border-b border-slate-800 shadow-2xl p-6 pt-24 animate-slide-down">
                        ...
                        <div className="mt-8 pt-6 border-t border-slate-800">
                            <p className="text-slate-500 text-sm mb-4 text-center">Suivez-moi</p>
